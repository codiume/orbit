    ...
    hooks: {
      ...
      'astro:build:done': async ({ dir, pages, logger }) => {
        ...
        if (!outDir || !config.build.format || !config.build.assets) {
          logger.warn(
            `${PKG_NAME} requires the 'outDir', 'build.format', 'build.assets' astro.config options. Skipping.`
          );
          return;
        }
        ...
        let purgedCssFiles = await Promise.all(
          purged
            ...
            .map(async ({ css, file }) => {
              let oldFile = file as string;
              let newFile = file as string;
              ...
              newFile = generateFileHash(oldFile, css);
